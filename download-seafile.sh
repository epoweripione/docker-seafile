...
curl=$?
if [ $wget -eq 0 ]; then
    addr=$(wget https://www.seafile.com/en/download/ -O - | grep -o -P "${regexp}")
    [ -n "${addr}" ] && wget "${addr}"
elif [ $curl -eq 0 ]; then
    addr=$(curl -Ls https://www.seafile.com/en/download/ | grep -o -P "${regexp}")
    [ -n "${addr}" ] && curl -Ls -O "${addr}"
else
...
file=$(echo "${addr}" | awk -F/ '{ print $NF }' | sort -r | head -1)
